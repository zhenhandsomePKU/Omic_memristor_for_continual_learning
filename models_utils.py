...
def quantization_hardware(input_tensor,bit_num=6,upper_bound=5,noise_std=0.1,device='cpu'):
    #对输入的张量进行指定bit数目的量化

    interval=upper_bound/(2**bit_num-1)
    half_interval=interval/2
    clamp_tensor=torch.clamp(input_tensor, min=(-1)*upper_bound-half_interval, max=upper_bound+half_interval).to(device)
...
def real_Binarize(tensor,interval=0.001,slope=0.001):   #自己定义一个新的二值化的方程  根据单元结构进行拟合得出来的
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            for p in group['params']:
                ...
                #decayed_exp_avg = torch.where(p.data.abs()>group['meta'], torch.zeros_like(p.data)*exp_avg, exp_avg)
...
def train(model, train_loader, current_task_index, optimizer, device, args,
    ...
    if quantization_use:
        for p in list(model.parameters()):  # updating the org attribute
            if hasattr(p, 'org'):
                p.org.copy_(quantization_hardware(p.data, bit_num=args.bit_num, upper_bound=args.upper_bound, noise_std=args.noise_std, device=device))


# consider the line resistance

'''
def set_line_res_matrix(row_num, column_num, res_unit, ratio ):
    # res_unit: 单段线阻与单个器件的LRS的归一化的结果
...
line_resistance_matrix_0=set_line_res_matrix(500,784,2.191e-5,1.7)
line_resistance_matrix_1=set_line_res_matrix(200,500,2.191e-5,1.7)
line_resistance_matrix_2=set_line_res_matrix(10,200,2.191e-5,1.7)
